...
This skill's reference code below uses **async SQLAlchemy + SQLite + `create_all()`**. The Cadence
stack is different and **takes precedence**. When applying this skill in this project, use these
patterns instead of the ones further down:

| Topic | Skill default (below) | **Use for Cadence** |
|---|---|---|
| Database | SQLite (`sqlite+aiosqlite`) | **PostgreSQL** via `postgresql+psycopg://` (psycopg 3) |
| SQLAlchemy | async (`AsyncSession`, `await db.execute`) | **sync** (`Session`, `db.execute`) |
| Schema management | `Base.metadata.create_all()` on startup | **Alembic** owns the schema — never `create_all` |
| Migrations skill | — | use the `db-migration` skill |

Everything else in this skill (project structure, Pydantic v2 schemas, JWT flow, CORS, 422 handling,
critical rules) applies **unchanged**.

### Dependencies (Cadence)

```bash
uv add "fastapi[standard]" "sqlalchemy>=2.0" alembic "psycopg[binary]" \
       pydantic-settings "passlib[bcrypt]" python-jose[cryptography]
```

### Database setup — sync + Postgres (replaces the async `database.py` below)
...
# e.g. postgresql+psycopg://user:pass@localhost:5432/cadence
...
### Routers — sync (drop `async`/`await` from the examples below)

```python
# src/items/router.py  (sync form)
...
Apply the same change to the auth router/dependencies: `def` instead of `async def`, `Session`
instead of `AsyncSession`, and no `await` on `db.execute` / `db.commit` / `db.refresh`.

### Schema management — Alembic, not `create_all()`

**Remove** the table-creation block from `lifespan` in `main.py`:

```python
# DELETE this — Alembic owns the schema in Cadence:
# async with engine.begin() as conn:
#     await conn.run_sync(Base.metadata.create_all)
```

Instead, one-time setup and per-change workflow:

```bash
uv add alembic
uv run alembic init migrations
# In migrations/env.py: set target_metadata = Base.metadata and read the URL from settings.
# Then, on every model change, use the db-migration skill:
uv run alembic revision --autogenerate -m "add items table"
uv run alembic upgrade head
```

### Timestamps — timezone-aware (avoid deprecated `datetime.utcnow`)
...
> Note: `python-jose` and `passlib` are largely unmaintained. They work fine, but `pyjwt` +
> `bcrypt` (or `pwdlib`) are modern alternatives if you'd rather not carry them.
